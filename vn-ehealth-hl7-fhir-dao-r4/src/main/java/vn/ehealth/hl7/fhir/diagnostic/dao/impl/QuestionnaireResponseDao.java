...
public class QuestionnaireResponseDao extends BaseDao<QuestionnaireResponseEntity, QuestionnaireResponse> {
	...
	protected String getProfile() {
		return "QuestionnaireResponse-v1.0";
	}
...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<QuestionnaireResponseEntity> entitys = mongo.find(query, QuestionnaireResponseEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (QuestionnaireResponseEntity item : entitys) {
				QuestionnaireResponse obj = transform(item);
	...
	public long countMatchesAdvancedTotal(
		...
		total = mongo.count(query, QuestionnaireResponseEntity.class);
		return total;
	}
