package vn.ehealth.hl7.fhir.definitionalartifact.dao.impl;
...
import vn.ehealth.hl7.fhir.definitionalartifact.entity.PlanDefinitionEntity;
...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<PlanDefinitionEntity> entitys = mongo.find(query, PlanDefinitionEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (PlanDefinitionEntity item : entitys) {
				PlanDefinition obj = transform(item);
	...
	public long countMatchesAdvancedTotal(
		...
		total = mongo.count(query, PlanDefinitionEntity.class);
		return total;
	}
