...
public class EpisodeOfCareDao extends BaseDao<EpisodeOfCareEntity, EpisodeOfCare> {
	...
	public long getTotal(FhirContext fhirContext, ReferenceParam careManager, ReferenceParam condition,
			DateRangeParam date, TokenParam identifier, ReferenceParam incomingreferral, ReferenceParam organization,
	...
	private Criteria setParamToCriteria(FhirContext fhirContext, ReferenceParam careManager, ReferenceParam condition,
			DateRangeParam date, TokenParam identifier, ReferenceParam incomingreferral, ReferenceParam organization,
			ReferenceParam patient, TokenParam status, TokenParam type, TokenParam resid, DateRangeParam _lastUpdated,
			TokenParam _tag, UriParam _profile, TokenParam _query, TokenParam _security, StringParam _content) {
		...
		if (organization != null) {
			if (organization.getValue().indexOf("|") == -1) {
				...
				criteria.and("managingOrganization.identifier.system").is(ref[0])
						.and("managingOrganization.identifier.value").is(ref[1]);
			}
		}
		// patient
		if (patient != null) {
