...
import vn.ehealth.hl7.fhir.base.entity.SubscriptionEntity;
import vn.ehealth.hl7.fhir.core.entity.BaseResource;
import vn.ehealth.hl7.fhir.core.util.ConstantKeys;
import vn.ehealth.hl7.fhir.dao.BaseDao;

@Repository
public class SubscriptionDao extends BaseDao<SubscriptionEntity, Subscription> {

	@Override
	protected String getProfile() {
		return "Subscription-v1.0";
	}
...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<SubscriptionEntity> entitys = mongo.find(query, SubscriptionEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (SubscriptionEntity item : entitys) {
				Subscription obj = transform(item);
	...
	public long countMatchesAdvancedTotal(
		...
		total = mongo.count(query, SubscriptionEntity.class);
		return total;
	}
