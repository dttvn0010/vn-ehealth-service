...
public class PractitionerDao extends BaseDao<PractitionerEntity, Practitioner> {
	...
	public List<Resource> search(FhirContext fhirContext, StringParam address, StringParam addressCity,
			...
			TokenParam _security, StringParam _content, ReferenceParam managingOrg, StringParam _page, String sortParam,
			Integer count) {
	...
	public long countMatchesAdvancedTotal(FhirContext fhirContext, StringParam address, StringParam addressCity,
		...
		criteria = setParamToCriteria(address, addressCity, addressCountry, addressPostalCode, addressState, addressUse,
				communication, email, family, gender, given, identifier, name, phone, phonetic, telecom, resid,
	...
	private Criteria setParamToCriteria(StringParam address, StringParam addressCity, StringParam addressCountry,
			StringParam addressPostalCode, StringParam addressState, TokenParam addressUse, TokenParam communication,
			TokenParam email, StringParam family, TokenParam gender, StringParam given, TokenParam identifier,
			StringParam name, TokenParam phone, StringParam phonetic, TokenParam telecom, TokenParam resid,
			DateRangeParam _lastUpdated, TokenParam _tag, UriParam _profile, TokenParam _query, TokenParam _security,
			StringParam _content, ReferenceParam managingOrg) {
		Criteria criteria = null;
		...
		if (managingOrg != null) {
			...
			if (managingOrg.getValue().indexOf("|") == -1) {
				criteria.orOperator(
						Criteria.where("extension.value.reference.myStringValue").regex(managingOrg.getValue()),
