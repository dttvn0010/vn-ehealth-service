...
import vn.ehealth.hl7.fhir.diagnostic.entity.BodyStructureEntity;
...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<BodyStructureEntity> entitys = mongo.find(query, BodyStructureEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (BodyStructureEntity item : entitys) {
				BodyStructure obj = transform(item);
	...
	public long countMatchesAdvancedTotal(
		...
		total = mongo.count(query, BodyStructureEntity.class);
		return total;
	}
