...
import vn.ehealth.hl7.fhir.careprovision.entity.RequestGroupEntity;
import vn.ehealth.hl7.fhir.core.entity.BaseResource;
import vn.ehealth.hl7.fhir.core.util.ConstantKeys;
import vn.ehealth.hl7.fhir.dao.BaseDao;

@Repository
public class RequestGroupDao extends BaseDao<RequestGroupEntity, RequestGroup> {

	@Override
	protected String getProfile() {
		return "RequestGroup-v1.0";
	}

	@Override
	protected Class<? extends BaseResource> getEntityClass() {
		return RequestGroupEntity.class;
	}
...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<RequestGroupEntity> entitys = mongo.find(query, RequestGroupEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (RequestGroupEntity item : entitys) {
				RequestGroup obj = transform(item);
	...
	public long countMatchesAdvancedTotal(
		...
		total = mongo.count(query, RequestGroupEntity.class);
		return total;
	}
