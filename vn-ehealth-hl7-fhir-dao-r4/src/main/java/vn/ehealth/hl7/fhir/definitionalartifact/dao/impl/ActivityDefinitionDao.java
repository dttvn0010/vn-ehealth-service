...
public class ActivityDefinitionDao extends BaseDao<ActivityDefinitionEntity, ActivityDefinition> {
	...
	protected Class<? extends BaseResource> getEntityClass() {
		return ActivityDefinitionEntity.class;
	}
...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<ActivityDefinitionEntity> entitys = mongo.find(query, ActivityDefinitionEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (ActivityDefinitionEntity item : entitys) {
				ActivityDefinition obj = transform(item);
