...
public class VisionPrescriptionDao extends BaseDao<VisionPrescriptionEntity, VisionPrescription> {
	...
	protected Class<? extends BaseResource> getEntityClass() {
		return VisionPrescriptionEntity.class;
	}

	@Override
	protected Class<? extends DomainResource> getResourceClass() {
		return VisionPrescription.class;
	}
...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<VisionPrescriptionEntity> entitys = mongo.find(query, VisionPrescriptionEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (VisionPrescriptionEntity item : entitys) {
				VisionPrescription obj = transform(item);
	...
	public long countMatchesAdvancedTotal(
		...
		total = mongo.count(query, VisionPrescriptionEntity.class);
		return total;
	}
