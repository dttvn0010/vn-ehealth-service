...
public class CareTeamDao extends BaseDao<CareTeamEntity, CareTeam> {
	...
	public List<Resource> search(FhirContext fhirContext, TokenParam category, ReferenceParam context,
			DateRangeParam date, ReferenceParam encounter, TokenParam identifier, ReferenceParam participant,
			ReferenceParam patient, TokenParam status, ReferenceParam subject,
			// COMMON
			TokenParam resid, DateRangeParam _lastUpdated, TokenParam _tag, UriParam _profile, TokenParam _query,
			TokenParam _security, StringParam _content, NumberParam _page, String sortParam, Integer count) {
		List<Resource> resources = new ArrayList<>();
		Criteria criteria = setParamToCriteria(category, context, date, encounter, identifier, participant, patient,
	...
	public long getTotal(FhirContext fhirContext, TokenParam category, ReferenceParam context, DateRangeParam date,
			ReferenceParam encounter, TokenParam identifier, ReferenceParam participant, ReferenceParam patient,
	...
	private Criteria setParamToCriteria(TokenParam category, ReferenceParam context, DateRangeParam date,
			ReferenceParam encounter, TokenParam identifier, ReferenceParam participant, ReferenceParam patient,
			TokenParam status, ReferenceParam subject, TokenParam resid, DateRangeParam _lastUpdated, TokenParam _tag,
			UriParam _profile, TokenParam _query, TokenParam _security, StringParam _content) {
		...
		if (participant != null) {
			if (participant.getValue().indexOf("|") == -1) {
				...
				criteria.and("participant.member.identifier.system").is(ref[0])
						.and("participant.member.identifier.value").is(ref[1]);
			}
		}
		// status
		if (status != null) {
