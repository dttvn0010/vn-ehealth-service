...
//		var includeMap = getIncludeMap(ResourceType.Bundle, keys, includes);
		...
		List<ProvenanceEntity> entitys = mongo.find(query, ProvenanceEntity.class);
		if (entitys != null && entitys.size() > 0) {
			for (ProvenanceEntity item : entitys) {
				Provenance obj = transform(item);
	...
	public long countMatchesAdvancedTotal(
		...
		total = mongo.count(query, ProvenanceEntity.class);
		return total;
	}
