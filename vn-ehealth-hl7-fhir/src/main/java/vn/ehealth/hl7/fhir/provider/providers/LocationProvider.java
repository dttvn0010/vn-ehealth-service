...
public class LocationProvider implements IResourceProvider {
    ...
    public MethodOutcome createLocation(HttpServletRequest theRequest, @ResourceParam Location obj) {
        ...
        try {
            ...
            method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Create succsess",
                    "urn:uuid: " + mongoLocation.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE, myString));
    ...
    @Search
    public List<Resource> searchLocation(HttpServletRequest request,
        ...
        if (count != null && count > 50) {
            ...
            if (theSort != null) {
                ...
                List<Resource> results = locationDao.search(fhirContext, address, addressCity, addressCountry, addressState,
                        endpoint, identifier, name, near, nearDistance, operationalStatus, organization, partof, status,
                        type, resid, _lastUpdated, _tag, _profile, _query, _security, _content, _page, sortParam,
                        count);
                return results;
            }
            List<Resource> results = locationDao.search(fhirContext, address, addressCity, addressCountry, addressState,
                    endpoint, identifier, name, near, nearDistance, operationalStatus, organization, partof, status,
                    type, resid, _lastUpdated, _tag, _profile, _query, _security, _content, _page, "", count);
            return results;
        }
    }
    ...
    public MethodOutcome updateLocation(@IdParam IdType theId, @ResourceParam Location patient) {
        ...
        method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Update succsess",
                "urn:uuid: " + newLocation.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE));
    ...
    public Parameters findMatchesAdvancedTotal(HttpServletRequest request,
        ...
        long total = locationDao.findMatchesAdvancedTotal(fhirContext, address, addressCity, addressCountry, addressState,
                endpoint, identifier, name, near, nearDistance, operationalStatus, organization, partof, status, type,
