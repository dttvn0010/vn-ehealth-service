...
public class MedicationStatementProvider implements IResourceProvider {
    ...
    public MethodOutcome createMedicationStatement(HttpServletRequest theRequest,
        ...
        try {
            ...
            method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Create succsess",
                    "urn:uuid: " + mongoMedicationStatement.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE,
                    myString));
    ...
    @Search
    public List<Resource> searchMedicationStatement(HttpServletRequest request,
        ...
        if (count != null && count > 50) {
            ...
            if (theSort != null) {
                ...
                List<Resource> results = medicationStatementDao.search(fhirContext, active, category, code, context,
                        effective, identifier, medication, partOf, patient, source, status, subject, resid,
                        _lastUpdated, _tag, _profile, _query, _security, _content, _page, sortParam, count);
                return results;
            }
            List<Resource> results = medicationStatementDao.search(fhirContext, active, category, code, context,
                    effective, identifier, medication, partOf, patient, source, status, subject, resid, _lastUpdated,
    ...
    @Update
    public MethodOutcome updateMedicationStatement(@IdParam IdType theId, @ResourceParam MedicationStatement patient) {
        ...
        try {
            newMedicationStatement = medicationStatementDao.update(fhirContext, patient, theId);
        } catch (Exception ex) {
            if (ex instanceof OperationOutcomeException) {
                OperationOutcomeException outcomeException = (OperationOutcomeException) ex;
                method.setOperationOutcome(outcomeException.getOutcome());
        ...
        method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Update succsess",
                "urn:uuid: " + newMedicationStatement.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE));
