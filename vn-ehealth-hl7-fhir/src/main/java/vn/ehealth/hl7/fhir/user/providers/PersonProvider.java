...
public class PersonProvider implements IResourceProvider {
    ...
    public MethodOutcome createPerson(HttpServletRequest theRequest, @ResourceParam Person obj) {
        ...
        try {
            ...
            method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Create succsess",
                    "urn:uuid: " + mongoPerson.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE, myString));
    ...
    @Search
    public List<Resource> searchPerson(HttpServletRequest request,
            ...
            @OptionalParam(name = ConstantKeys.SP_PAGE) StringParam _page,@Sort SortSpec theSort,
            @Count Integer count) throws OperationOutcomeException {
        if (count != null && count > 50) {
            ...
            if (theSort != null) {
                ...
                List<Resource> results = personDao.search(fhirContext, active, address, addressCity, addressCountry, addressState, birthDate, email, gender, identifier, name, patient, phone, phonetic, telecom, resid, 
                        _lastUpdated, _tag, _profile, _query, _security, _content,managingOrg, _page, sortParam, count);
                return results;
            }
            List<Resource> results = personDao.search(fhirContext, active, address, addressCity, addressCountry, addressState, birthDate, email,
                    gender, identifier, name, patient, phone, phonetic, telecom, resid, _lastUpdated, _tag, _profile, _query, _security, _content,managingOrg, _page, "", count);
            return results;
        }
    }

    @Delete
    public Person deletePerson(HttpServletRequest request, @IdParam IdType internalId) {
    ...
    @Update
    public MethodOutcome updatePerson(@IdParam IdType theId, @ResourceParam Person patient) {
        ...
        try {
            newPerson = personDao.update(fhirContext, patient, theId);
        } catch (Exception ex) {
            if (ex instanceof OperationOutcomeException) {
                OperationOutcomeException outcomeException = (OperationOutcomeException) ex;
                method.setOperationOutcome(outcomeException.getOutcome());
        ...
        method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Update succsess",
                "urn:uuid: " + newPerson.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE));
    ...
    public Parameters findMatchesAdvancedTotal(HttpServletRequest request,
        ...
        long total = personDao.findMatchesAdvancedTotal(fhirContext, active, address, addressCity, addressCountry, addressState, birthDate, 
                email, gender, identifier, name, patient, phone, phonetic, telecom, resid, _lastUpdated, _tag, _profile, _query, _security, _content,managingOrg);
