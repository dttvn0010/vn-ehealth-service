...
public class DiagnosticReportProvider implements IResourceProvider {
    ...
    public MethodOutcome createDiagnosticReport(HttpServletRequest theRequest, @ResourceParam DiagnosticReport obj) {
        ...
        try {
            ...
            method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Create succsess",
                    "urn:uuid: " + mongoDiagnosticReport.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE));
    ...
    @Search
    public List<Resource> searchDiagnosticReport(HttpServletRequest request,
        ...
        if (count != null && count > 50) {
            ...
            if (theSort != null) {
                ...
                List<Resource> results = diagnosticReportDao.search(fhirContext, active, basedOn, category, code, conetext,
                        date, diagnosis, encounter, identifier, image, issued, patient, performer, result, specimen,
                        status, subject, resid, _lastUpdated, _tag, _profile, _query, _security, _content, _page,
                        sortParam, count);
                return results;
            }
            List<Resource> results = diagnosticReportDao.search(fhirContext, active, basedOn, category, code, conetext, date,
    ...
    public MethodOutcome updateDiagnosticReport(@IdParam IdType theId, @ResourceParam DiagnosticReport patient) {
        ...
        method.setOperationOutcome(OperationOutcomeFactory.createOperationOutcome("Update succsess",
                "urn:uuid: " + newDiagnosticReport.getId(), IssueSeverity.INFORMATION, IssueType.INCOMPLETE));
