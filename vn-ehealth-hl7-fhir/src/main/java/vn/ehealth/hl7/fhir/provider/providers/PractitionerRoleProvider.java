    ...
    public List<Resource> searchPractitioner(HttpServletRequest request,
        ...
        if (count != null && count > 50) {
            ...
            if (theSort != null) {
                ...
                List<Resource> results = practitionerRoleDao.search(fhirContext, active, date, email, endpoint,
                        identifier, location, organization, phone, practitioner, role, service, specialty, telecom,
                        resid, _lastUpdated, _tag, _profile, _query, _security, _content, _page, sortParam, count);
                return results;
            }
            List<Resource> results = practitionerRoleDao.search(fhirContext, active, date, email, endpoint, identifier,
                    location, organization, phone, practitioner, role, service, specialty, telecom, resid, _lastUpdated,
                    _tag, _profile, _query, _security, _content, _page, "", count);
            return results;
        }
    }

    @Delete
    public PractitionerRole removePractitioner(HttpServletRequest request, @IdParam IdType internalId) {
