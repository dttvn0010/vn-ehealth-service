...
public class DataConvertUtil {
	...
	static List<Method> getAddMethods(Class<?> clazz, String fieldName) {
		...
		for (var method : clazz.getMethods()) {
			if (method.getParameterCount() != 1)
				continue;
			if (!addMethodName.equals(method.getName()))
				continue;
			methods.add(method);
		}
		return methods;
	...
	static Map<Class<?>, Class<?>> objClassToPrimitive = Map.of(Integer.class, int.class, Long.class, long.class,
			Short.class, short.class, Double.class, double.class, Boolean.class, boolean.class);

	public static boolean isAssignable(Class<?> fromClass, Class<?> toClass) {
		if (fromClass == null || toClass == null)
			return false;
		return fromClass.equals(toClass) || fromClass.equals(objClassToPrimitive.get(toClass))
				|| toClass.equals(objClassToPrimitive.get(fromClass)) || toClass.isAssignableFrom(fromClass);
	}
	...
	private static boolean isObjectClass(Class<?> clazz) {
		...
		if (String.class.equals(clazz))
			return false;
		if (objClassToPrimitive.containsKey(clazz))
			return false;
		if (objClassToPrimitive.containsValue(clazz))
			return false;
	...
	private static Object getEntFieldValue(Object obj, String fieldName) {
		...
		try {
			...
			throw new RuntimeException(
					String.format("Cannot get field \"%s\" for class \"%s\"", fieldName, obj.getClass().getName()), e);
		}
	}

	private static void setEntFieldValue(Object ent, String fieldName, Object fhirVal) {
		if (ent == null || fhirVal == null)
			return;
		...
		try {
			...
			throw new RuntimeException(
					String.format("Cannot set field \"%s\" for class \"%s\"", fieldName, ent.getClass().getName()), e);
		}
	}
	...
	private static Object getFhirFieldValue(Object obj, String fieldName) {
		...
		try {
			...
			if (hasFieldMethod != null)
				hasField = (Boolean) hasFieldMethod.invoke(obj);
			...
			if (hasField != null && hasField) {
				...
				if (fieldName.endsWith("_")) {
					...
					try {
						...
						logger.error(String.format("Fail to execute method %s for class %s", getter.getName(),
	...
	private static void addFhirFieldValue(Object obj, String fieldName, Object entVal) {
		...
		throw new RuntimeException(
				String.format("Cannot addvalue \"%s\" for class \"%s\"", fieldName, obj.getClass().getName()));
	}
	...
	private static void setFhirFieldValue(Object obj, String fieldName, Object entVal) {
		...
		throw new RuntimeException(
				String.format("Cannot set field \"%s\" for class \"%s\"", fieldName, obj.getClass().getName()));
	}
	...
	private static Object stringToCode(Class<?> fhirCodeType, String code) {
		try {
			...
			throw new RuntimeException(
					String.format("Cannot convert code \"%s\" to class \"%s\"", code, fhirCodeType.getName()), e);
		}
	}
	...
	private static String codeToString(Object fhirCode) {
		...
		try {
			...
			throw new RuntimeException(String.format("Cannot convert fhirCode \"%s\" to String \"%s\"", fhirCode), e);
		}
	}

	public static Meta getMeta(BaseResource entity) {
		if (entity == null)
			return null;
...
//        }
	...
	public static <T> T fhirToEntity(Object obj, Class<T> entType) {
		...
		if (String.class.equals(entType)) {
			...
			if (objValue != null && objValue.getClass().isEnum())
				return (T) codeToString(objValue);
		}
	...
	public static <T> T entityToFhir(Object ent, Class<T> fhirType) {
		if (ent == null || fhirType == null)
			return null;

		// Direct assignment
		if (isAssignable(ent.getClass(), fhirType))
			return (T) ent;
