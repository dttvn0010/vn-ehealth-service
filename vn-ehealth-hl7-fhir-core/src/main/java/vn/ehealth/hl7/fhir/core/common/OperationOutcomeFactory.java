...
public class OperationOutcomeFactory {
    ...
    public static OperationOutcome createOperationOutcome(String diagnostics, String message, IssueSeverity severity , IssueType issueType,List<String> theLocation) {
        OperationOutcome outcome = new OperationOutcome();
        List<StringType> location = new ArrayList<>(); 
        for (String string : theLocation) {
            location.add(new StringType(string));
        }
        outcome.addIssue()
                .setCode(issueType)
                .setSeverity(severity)
